...
public class UsersmanagmentController implements Initializable {
   ...
   private final ObservableList<Users> dataList = FXCollections.observableArrayList();
    ...
    public void initialize(URL url, ResourceBundle rb) {
          UsersCRUD es = new UsersCRUD();
        List Users = es.listerUsers();
        ObservableList liste = FXCollections.observableArrayList(Users);
        tableusers.setItems(liste);
         colname.setCellValueFactory(new PropertyValueFactory<>("id"));
                         collastname.setCellValueFactory(new PropertyValueFactory<>("name"));
        colemail.setCellValueFactory(new PropertyValueFactory<>("email"));
        

       
       // eventstable.setItems(liste);*/
      FilteredList<Users> filteredData = new FilteredList<>(liste, b -> true);

recherche.textProperty().addListener((observable, oldValue, newValue) -> {
filteredData.setPredicate(new Predicate<Users>() {
                            @Override
                            public boolean test(Users users) {
                                ...
                                if (users.getName().toLowerCase().indexOf(lowerCaseFilter) != -1 ) {
                                    return true;
